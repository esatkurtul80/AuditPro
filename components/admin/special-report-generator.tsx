...
export function SpecialReportGenerator({ audit, store, mode = 'download', onComplete, onError, onClose, headerOffset = 0 }: SpecialReportGeneratorProps) {
    ...
    const generatePDF = async () => {
        ...
        // Snapshot current style (transform dahil — mobil scale'i sıfırlamak için)
        const originalStyle = {
        ...
        // Normalize for clean rendering — transform sıfırlanmazsa
        // html2canvas'ın y/height koordinatları tablet scale'i nedeniyle kayar
        ...
        // Sadece küçük thumbnail resimlere data-original-url ata
        const imgEls = Array.from(element.querySelectorAll('a[href] > img')) as HTMLImageElement[];
        imgEls.forEach(img => {
            const anchor = img.closest('a') as HTMLAnchorElement | null;
            if (anchor?.href) img.setAttribute('data-original-url', anchor.href);
        });

        // Gerçek içerik yüksekliğini spacer EKLEMEDEN önce kaydet
        const realContentHeight = element.scrollHeight;

        // İki frame bekle — layout tam oturması için
        await new Promise<void>(r => requestAnimationFrame(() => requestAnimationFrame(() => r())));

        const fileName = `Ozel_Rapor_${audit.storeName}_${format(new Date(), "yyyy-MM-dd")}.pdf`;

        // SCALE her zaman 2 — her sayfayı ayrı yakalıyoruz, limit sorunu yok
        const SCALE       = 2;
        const PAGE_H_CSS  = 12000; // CSS px / PDF sayfası
        ...
        try {
            ...
            // Küçük thumbnail link koordinatları
            const reportRect = element.getBoundingClientRect();
            const imageLinks: { x: number; y: number; w: number; h: number; url: string }[] = [];
            element.querySelectorAll('img[data-original-url]').forEach((img: any) => {
            ...
            // İlk sayfanın yüksekliğini hesapla
            ...
            // ── Her sayfayı ayrı capture et (SCALE=2 sabit, canvas limit yok) ──
            for (let i = 0; i < pages; i++) {
                ...
                // Bu sayfaya düşen thumbnail linkleri ekle
                const pageTopCss = yStart;
                const pageBotCss = yStart + chunkH;
                imageLinks.forEach(lnk => {
                    if (lnk.y + lnk.h < pageTopCss || lnk.y > pageBotCss) return;
                    const relY = lnk.y - pageTopCss;
                    pdf.link(
                        +(lnk.x * 0.264583).toFixed(2),
                        +(relY   * 0.264583).toFixed(2),
                        +(lnk.w  * 0.264583).toFixed(2),
                        +(lnk.h  * 0.264583).toFixed(2),
                        { url: lnk.url }
                    );
                });
            }

            imgEls.forEach(img => img.removeAttribute('data-original-url'));
            pdf.save(fileName);
            onComplete?.();
    ...
    return (
        <>
            ...
            <style jsx global>{`
                ...
                .generator-wrapper .section-card { margin-bottom: 25px; border: var(--border-style); }
