...
import { Store, AccommodationType, LeaveType } from "@/lib/types";
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const leaveQuery = query(collection(db, "leave_types"));
                ...
                // Execute in parallel
                const [accSnap, leaveSnap, storesSnap, scheduleSnap] = await Promise.all([
                    getDocs(accQuery),
                    getDocs(leaveQuery),
                    getDocs(storesQuery),
                ...
                // 1. Process Accommodation Types
                const accTypes = accSnap.docs.map(d => ({ id: d.id, ...d.data() } as AccommodationType));
                setAccommodationTypes(accTypes);

                // 2. Process Leave Types
                const leaveTypesData = leaveSnap.docs.map(d => ({ id: d.id, ...d.data() } as LeaveType));
                ...
                // 4. Process Schedule
                const items: ScheduleItem[] = scheduleSnap.docs.map(doc => {
                    const data = doc.data();
                    return {
                        id: doc.id,
                        ...data,
            ...
            >
                ...
                <div className="flex flex-col gap-1 mt-1">
                    {items.map((item, i) => {
                        ...
                        let bgColor = "bg-blue-600 text-white border-blue-700 shadow-sm";
                        let customStyle = {};

                        if (item.type === 'leave') {
                            const leaveType = item.leaveTypeId ? leaveTypes.find(lt => lt.id === item.leaveTypeId) : null;
                            if (leaveType && leaveType.color) {
                                // Use custom color with some transparency for bg, and solid for text/border
                                customStyle = {
                                    backgroundColor: `${leaveType.color}20`, // 20 hex = ~12% opacity
                                    color: leaveType.color,
                                    borderColor: `${leaveType.color}40`
                                };
                                bgColor = ""; // Reset default classes
                            } else {
                        ...
                        return (
                            <div
                                key={item.id}
                                className={cn(
                                    "text-xs px-1.5 py-1 rounded-md border whitespace-normal break-words h-auto font-semibold transition-all hover:scale-[1.02]",
                                    bgColor
                                )}
                                style={customStyle}
                            >
                                {item.type === 'leave' ? (
                                    <span className="flex items-center gap-1">
                                        ...
                                        {(() => {
                                            const leaveType = item.leaveTypeId ? leaveTypes.find(lt => lt.id === item.leaveTypeId) : null;
                                            return leaveType ? leaveType.name : 'İzin';
                                        })()}
                                    </span>
                                ) : (
        ...
        <DashboardLayout>
            <div className="w-full py-6 px-4 md:px-6">
                ...
                <div className="flex flex-col lg:flex-row gap-4 items-start">
                    ...
                    {/* Calendar Grid */}
                    <div className="w-full lg:flex-1 bg-white rounded-xl border shadow-sm overflow-hidden">
                        {/* Days Header */}
                        ...
                        {/* Calendar Body */}
                        <div className="grid grid-cols-7 auto-rows-auto overflow-y-auto content-start">
                    ...
                    <div className="w-full lg:w-80 shrink-0">
                        <Card className="p-4 border-l-4 border-l-blue-600 shadow-sm overflow-y-auto max-h-[600px]">
                            ...
                            <div className="space-y-4">
                                {selectedItems.length === 0 ? (
                                    ...
                                    selectedItems.map((item, index) => {
                                        ...
                                        if (item.type === 'leave') {
                                            const leaveType = item.leaveTypeId ? leaveTypes.find(lt => lt.id === item.leaveTypeId) : null;
                                            const leaveColor = leaveType?.color || '#a855f7'; // fallback purple-500

                                            return (
                                                <div key={item.id} className="bg-slate-50 rounded-lg p-4 border border-slate-100 relative overflow-hidden">
                                                    <div className="absolute left-0 top-0 bottom-0 w-1" style={{ backgroundColor: leaveColor }}></div>
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        <div>
                                                            <h3 className="font-bold text-slate-900">{leaveType ? leaveType.name : 'İzin'}</h3>
                                                            {item.note && (
                                                        ...
                                                        <div className="flex items-center text-xs text-slate-500 mb-3">
                                                            ...
                                                            {store.location && (
                                                                <a
                                                                    ...
                                                                    className="ml-2 flex items-center gap-1.5 px-2 py-0.5 bg-blue-50 text-blue-700 rounded-md border border-blue-200 hover:bg-blue-100 transition-colors text-xs font-semibold no-underline shadow-sm"
                                                                >
