...
6. SÜREGELEN ORTAK KONULARIN BELİRTİLMESİ (ÇOK ÖNEMLİ): Bir sorunun ardışık hata sayısı 2 veya daha fazla olsa bile, eğer "Şimdiki Not" ile "Geçmiş Denetimlerde Alınan Notlar" arasında ortak/aynı bir ürün (örn. Kuşburnu, Kaju vb.), ortak bir marka, ortak bir parti no (örn. 21.03 pt) veya ortak çok spesifik bir detay GEÇMİYORSA, bunu kesinlikle süregelen ortak sorun olarak KABUL ETME ve "Önceki Denetimlerden Süregelen Eksiklikler" listesine kesinlikle EKLEME. 
Örneğin geçmiş denetimde "Fıstık", şimdiki denetimde "Kuşburnu böceklenme" yazıyorsa, soru aynı olsa bile detaylar uyuşmadığı için bunu süregelen eksiklik olarak yazma. Sadece hem geçmiş hem şimdiki notta birebir uyuşan/ortak olan spesifik ürün veya konuyu listele (Örn: "- [Ortak Ürün/Konu Detayı] (X denetimdir üst üste giderilmemiş)"). Ortak detay yoksa listeyi ve başlığı asla ekleme.`;
...
ÖNEMLİ: Eğer ardışık hata sayısı 2 veya daha fazla (consecutiveFailCount >= 2) olan süregelen maddeler varsa ve bu maddelerin şimdiki notu ile geçmiş notlarında ORTAK/AYNI ürün, parti no, marka veya konu detayı yer alıyorsa, bunları ana paragrafın ardından boş bir satır bırakarak "Önceki Denetimlerden Süregelen Eksiklikler:" başlığı altında liste (madde) halinde belirt (Örn: "- [Ortak Ürün/Konu Detayı] (X denetimdir üst üste giderilmemiş)"). Şimdiki not ile geçmiş notlar arasında ortak/aynı olan hiçbir detay (ürün adı, parti no vb.) yoksa bu listeyi ve başlığı kesinlikle ekleme.`;
