            ...
            while (attempts < maxAttempts) {
                try {
                    ...
                    if (isTransient && attempts < maxAttempts) {
                        console.warn(`[Groq Store] ${modelName} transient error (${err?.message}). Retrying in ${attempts * 400}ms...`);
                        await new Promise(resolve => setTimeout(resolve, attempts * 400));
                    } else {
                        console.warn(`[Groq Store] ${modelName} failed on attempt ${attempts} (${err?.message}), trying next model...`);
                        break;
                    }
                }
            }
