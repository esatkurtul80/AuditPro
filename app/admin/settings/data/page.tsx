...
import { collection, getCountFromServer, getDocs, writeBatch, query, limit, deleteDoc, doc } from "firebase/firestore";
...
export default function DataManagementPage() {
  ...
  const [viewingCollection, setViewingCollection] = useState<{ id: string, name: string } | null>(null);
  ...
  const handleViewDetail = async (colId: string, colName: string) => {
    setViewingCollection({ id: colId, name: colName });
    setCollectionData([]);
    setSearchQuery("");
    setViewModalOpen(true);
    setLoadingView(true);
    try {
      const q = query(collection(db, colId), limit(50));
      const snapshot = await getDocs(q);
  ...
  const confirmDeleteSingleDoc = async () => {
    if (!itemToDelete || !viewingCollection) return;
    ...
    try {
      ...
      setCollectionData(prev => prev.filter(item => item.id !== docId));
      setCounts(prev => ({
        ...prev,
      ...
      toast.error(`Kayıt silinemedi: ${error.message}`);
    } finally {
                ...
                <div key={col.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm transition-all hover:border-red-200 hover:shadow-red-50/50 flex flex-col">
                  ...
                  <div className="bg-slate-50 border-t border-slate-100 p-4 flex items-center justify-between gap-4 mt-auto">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        onClick={() => handleViewDetail(col.id, col.name)}
                        className="bg-white hover:bg-slate-100 text-slate-700 font-medium"
                      >
      ...
      <Dialog open={viewModalOpen} onOpenChange={setViewModalOpen}>
        <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              {viewingCollection?.id} koleksiyonundaki verilerin önizlemesi (Maksimum 50 kayıt listelenir).
            </DialogDescription>
          </DialogHeader>
          <div className="px-4 py-2 border-b">
            <Input
              placeholder="Veriler içinde ara (ID, Ad, Tanım, JSON formatında)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-100 border-none focus-visible:ring-1 focus-visible:ring-slate-300"
            />
          </div>
          <div className="flex-1 overflow-y-auto bg-slate-950 rounded-b-md p-4">
            {loadingView ? (
              ...
              <div className="h-40 flex items-center justify-center text-slate-400">
                Bu koleksiyonda hiç veri bulunamadı.
              </div>
            ) : (
              <div className="space-y-4">
                {collectionData
                  .filter(item => !searchQuery || JSON.stringify(item).toLowerCase().includes(searchQuery.toLowerCase().trim()))
                  .map((item) => (
                    <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-lg p-4 relative group">
      ...
      <AlertDialog open={!!itemToDelete} onOpenChange={(open) => !open && setItemToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bu işlem geri alınamaz. Seçtiğiniz belge (ID: <strong>{itemToDelete}</strong>) veritabanından tamamen silinecektir.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>İptal</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDeleteSingleDoc} className="bg-red-600 hover:bg-red-700 focus:ring-red-600">
